...
## TTS Server Integration (Google Cloud TTS)

### New: tts-server/ (Python FastAPI)
- OpenAI-compatible POST /v1/audio/speech endpoint returning MP3 with real Bangla bn-BD Neural2 voices.
- Auto language detection (bn/en/hi/ar) -> correct Google voice; female/male override.
- Byte-safe server-side chunking (Google 5000-byte cap), per-IP rate limiting, optional bearer-token auth, CORS.
- See tts-server/README.md for key setup + free deploy (Render/Railway).

### js/tts.js � server-first TTS with Web Speech fallback
- speak() calls the TTS server when configured; on ANY failure silently falls back to the built-in Web Speech API (existing behaviour never breaks).
- New: ttsServerUrl()/setTtsServerUrl(), ttsServerToken()/setTtsServerToken(), ttsServerEnabled().

### Settings & admin
- User Settings -> Text to Speech: TTS Server URL + Token fields (localStorage override).
- Admin Panel -> App Settings: TTS Server URL (public /system/settings) + TTS Server Token (signed-in /system_client/keys).

---

## TTS Server: switched to 100% FREE edge-tts (no API key)

- Removed Google Cloud TTS dependency (no GOOGLE_TTS_API_KEY, no billing).
- tts-server now uses edge-tts (Microsoft Edge neural voices) with real bn-BD voices: NabanitaNeural (female) / PradeepNeural (male).
- Same OpenAI-compatible POST /v1/audio/speech API � frontend unchanged.
