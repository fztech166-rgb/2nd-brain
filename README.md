...
- `js/supabase-init.js` persists the auth session (`persistSession: true`),
...
- All per-user data goes through `users/{uid}/notes`,
  `users/{uid}/...` paths. The RLS policies generated by
  `db/supabase-schema.sql` block every cross-user read.
...
- Supabase docs are mirrored to IndexedDB (`sb-supabase`) with a write
  queue; offline edits sync the moment connectivity returns.
...
1. **Drop the entire `2ndbrain/` folder onto any static host** (Netlify,
   Vercel, GitHub Pages, your own VPS, even a USB stick). There is no
   build step — everything is plain HTML / CSS / ES modules.
2. **Supabase project** — the app talks to a Supabase project through
   `js/supabase-init.js` (a Firestore-compatible adapter). Configure the
   project URL + anon key in `index.html` and `admin/admin.html`
   (`window.__SUPABASE__`).
3. **Database** — in Supabase Dashboard → SQL Editor, run the contents of
   `db/supabase-schema.sql`. It creates the `kv` document store + `admins`
   table, the full RLS policy set (a 1:1 port of `firestore.rules`) and
   the realtime publication.
4. **Admin emails** — `db/supabase-schema.sql` seeds `fztech166@gmail.com`
   into `admins`; add/edit the `insert into public.admins` line to match
